...
df = df_original.drop_duplicates(subset=['Participant', 'time', 'Sensor_ID', 'Meal', 'Photo_annotation', 'blood_analysis'])

#print len(df_original), len(df), '_____________'
...
#configure figure 
p = figure(x_axis_type="datetime", plot_width=1200, plot_height=400, title='participant %s' %(Participant_ID), active_drag='pan')
...
def update_show():
	...
	current = df[(df.index.values >= int(row_todelete)) & (df.time <= (df.iloc[int(row_todelete)]['time'] + pd.Timedelta('0 days 08:01:00')))]

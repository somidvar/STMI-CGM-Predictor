    ...
    if file.endswith('backup') or file.endswith('complete thread'):
        ...
        with open(file+'\WhatsApp Chat with %s.txt' %(Participant_ID), 'r') as f2:
			...
			for line in f2:
			    ...
			    #deal with the first message that can't be splitted by ' - Participant_ID'
				if line.endswith('Tap for more info.\n'):
					line = line.split(' - ')
					line = line[0] + ' - %s: '%(Participant_ID)  + line[1]
				
				#deal with message sent by CTRAL, so not an annotation
				line =  line.split(' - ')
				#if it can be splitted by ' - '
				try:
					if line[1].startswith('CTRAL'):
						line = line[0] + ' - %s: '%(Participant_ID)  + line[1]
					else:
						line = line[0] + ' - ' + line[1]
				#it can not 
				...
				line = line.split(' - %s: ' %(Participant_ID))
